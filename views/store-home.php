...
<select id="category" name="category" onchange="goToCatPage();">
	...
	<?php
$cat = new EcommerceCartProductCategory();
$cats = $cat->getModels();
?>
    ...
    $selected = '';
    if (isset($_GET['catid'])) {
        if ($_GET['catid'] == $c->id) $selected = 'selected="selected"';
    }
?>
...
<?php
if (EcommerceCartSetting::getValue("display_products") == "list") {
?>
...
?>  
        ...
        if (!empty($p->product_image_path)) {
            ...
            $path = $upload_dir['baseurl'] . '/ecommercecart/digproduct
...
?> 
        ...
        $attrs = array('id' => $p->id);
        $content = '';
        echo EcommerceCartSHortcodeManager::showCartButton($attrs, $content);
?>
				</div>
			</div>
		</div>  
