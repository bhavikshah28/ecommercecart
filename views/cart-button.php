...
$id = EcommerceCartCommon::getButtonId($data['product']->id);
?>
...
<?php if ($data['showPrice'] == 'only'): ?>
  <p class="SimpleEcommCartPrice" <?php echo $style; ?>>
  	...
  	<?php _e('Price', 'simpleecommcart'); ?><?php echo (EcommerceCartSetting::getValue('SIMPLEECOMMCART_CURRENCY_SYMBOL_text') != NULL) ? '(' . EcommerceCartSetting::getValue('SIMPLEECOMMCART_CURRENCY_SYMBOL_text') . ')' : '' ?>: <?php echo $data['price'] ?>
  
  </p>
<?php
else: ?>

  <form id='cartButtonForm_<?php echo $id ?>' class="SimpleEcommCartCartButton" method="post" action="<?php echo EcommerceCartCommon::getPageLink('store/cart'); ?>" <?php echo $data['style']; ?>>
...
?>
      ...
      <span style="font-weight:bold;font-size:1.01em;"><?php _e('Price', 'simpleecommcart'); ?><?php echo (EcommerceCartSetting::getValue('SIMPLEECOMMCART_CURRENCY_SYMBOL_text') != NULL) ? '(' . EcommerceCartSetting::getValue('SIMPLEECOMMCART_CURRENCY_SYMBOL_text') . ')' : '' ?>: <?php echo $data['price']; ?></span>
    <?php
    endif; ?>
    
    <?php if ($data['is_user_price'] == 1): ?>
      <div class="SimpleEcommCartUserPrice">
        <label for="SimpleEcommCartUserPriceInput_<?php echo $id ?>"><?php echo (EcommerceCartSetting::getValue('userPriceLabel')) ? EcommerceCartSetting::getValue('userPriceLabel') : __('Enter an amount: ') ?> </label><?php echo ECOMMERCECART_CURRENCY_SYMBOL ?><input id="SimpleEcommCartUserPriceInput_<?php echo $id ?>" name="item_user_price" value="<?php echo str_replace(SIMPLEECOMMCART_CURRENCY_SYMBOL, "", $data['price']); ?>" size="5">
      </div>
    <?php
    endif; ?>
...
?>
      <div class="SimpleEcommCartUserQuantity">
       <label for="SimpleEcommCartUserQuantityInput_<?php echo $id; ?>"><?php echo (EcommerceCartSetting::getValue('userQuantityLabel')) ? EcommerceCartSetting::getValue('userQuantityLabel') : __('Quantity: ') ?> </label>
       <input id="SimpleEcommCartUserQuantityInput_<?php echo $id; ?>" name="item_quantity" value="<?php echo $defaultQuantity; ?>" size="4">
      </div> 
    <?php
    elseif (is_numeric($data['quantity']) && $data['is_user_price'] != 1): ?>
