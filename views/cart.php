<?php
EcommerceCartSession::get('EcommerceCartCart')->resetPromotionStatus();
$items = EcommerceCartSession::get('EcommerceCartCart')->getItems();
//$shippingMethods = EcommerceCartSession::get('EcommerceCartCart')->getShippingMethods();
//$shipping = EcommerceCartSession::get('EcommerceCartCart')->getShippingCost();
//$promotion = EcommerceCartSession::get('EcommerceCartCart')->getPromotion();
$product = new EcommerceCartProduct();
$subtotal = EcommerceCartSession::get('EcommerceCartCart')->getSubTotal();
$discountAmount = EcommerceCartSession::get('EcommerceCartCart')->getDiscountAmount();
...
if (EcommerceCartSetting::getValue('continue_shopping') == 1) {
    // force the last page to be store home
    $lastPage = EcommerceCartSetting::getValue('store_url') ? EcommerceCartSetting::getValue('store_url') : get_bloginfo('url');
    EcommerceCartSession::set('SimpleEcommCartLastPage', $lastPage);
} else {
    if (isset($_SERVER['HTTP_REFERER']) && isset($_POST['task']) && $_POST['task'] == "addToCart") {
        $lastPage = $_SERVER['HTTP_REFERER'];
        EcommerceCartSession::set('SimpleEcommCartLastPage', $lastPage);
        if ($_POST['hascartwidget'] == 'yes') {
            header("Location: " . EcommerceCartSession::get('SimpleEcommCartLastPage'));
        }
    }
    if (!EcommerceCartSession::get('SimpleEcommCartLastPage')) {
...
if (isset($data['tax']) && $data['tax'] > 0) {
    ...
    $tax = EcommerceCartSession::get('EcommerceCartCart')->getTax('All Sales');
}
$cartImgPath = EcommerceCartSetting::getValue('cart_images_url');
...
<?php if (EcommerceCartSession::get('SimpleEcommCartInventoryWarning') && $fullMode): ?>
...
<?php if (EcommerceCartSession::get('SimpleEcommCartZipWarning')): ?>
  <div id="SimpleEcommCartZipWarning" class="SimpleEcommCartUnavailable">
    ...
    <?php
        EcommerceCartSession::drop('SimpleEcommCartZipWarning');
?>
    <input type="button" name="close" value="Ok" id="close" class="SimpleEcommCartButtonSecondary modalClose" />
  </div>
<?php
    elseif (EcommerceCartSession::get('SimpleEcommCartShippingWarning')): ?>
  <div id="SimpleEcommCartShippingWarning" class="SimpleEcommCartUnavailable">
    ...
    <?php EcommerceCartSession::drop('SimpleEcommCartShippingWarning'); ?>
    <input type="button" name="close" value="Ok" id="close" class="SimpleEcommCartButtonSecondary modalClose" />
  </div>
<?php
    endif; ?>

<?php if (EcommerceCartSession::get('SimpleEcommCartSubscriptionWarning')): ?>
  <div id="SimpleEcommCartSubscriptionWarning" class="SimpleEcommCartUnavailable">
    ...
    <?php
        EcommerceCartSession::drop('SimpleEcommCartSubscriptionWarning');
?>
    <input type="button" name="close" value="Ok" id="close" class="SimpleEcommCartButtonSecondary modalClose" />
  </div>
<?php
...
<?php
    if ($accountId = EcommerceCartCommon::isLoggedIn()) {
        ...
        if ($sub = $account->getCurrentAccountSubscription()) {
            if ($sub->isPayPalSubscription() && EcommerceCartSession::get('EcommerceCartCart')->hasPayPalSubscriptions()) {
?>
...
<form id='EcommerceCartCartForm' action="" method="post">
...
?>
      <tr>
        <td <?php if ($item->hasAttachedForms()) {
        ...
        <?php if ($fullMode): ?>
          <?php
            $removeItemImg = EcommerceCart_URL . '/images/remove-item.png';
            if ($cartImgPath) {
...
?>
        ...
        <?php
          ...
          <td <?php if ($item->hasAttachedForms()) {
                echo "class=\"noBottomBorder\"";
            } ?> colspan="2"><?php echo $item->getQuantity() ?></td>
        <?php
        endif; ?>
        <td <?php if ($item->hasAttachedForms()) {
            echo "class=\"noBottomBorder\"";
        } ?>><?php echo ECOMMERCECART_CURRENCY_SYMBOL ?><?php echo number_format($item->getProductPrice(), 2); ?></td>
        <td <?php if ($item->hasAttachedForms()) {
            echo "class=\"noBottomBorder\"";
        } ?>><?php echo ECOMMERCECART_CURRENCY_SYMBOL ?><?php echo number_format($price, 2) ?></td>
      </tr>
      <?php if ($item->hasAttachedForms()): ?>
        <tr>
          <td colspan="5">
            <a href='#' class="showEntriesLink" rel="<?php echo 'entriesFor_' . $itemIndex ?>"><?php _e('Show Details', 'simpleecommcart'); ?> <?php #echo count($item->getFormEntryIds());
             ?></a>
    ...
    <?php if (EcommerceCartSession::get('EcommerceCartCart')->requireShipping()): ?>
      
      <?php if (EcommerceCartSetting::getValue('use_live_rates')): ?>
        <?php $zipStyle = "style=''"; ?>
        
        <?php if ($fullMode): ?>
          <?php if (EcommerceCartSession::get('simpleecommcart_shipping_zip')): ?> 
            <?php $zipStyle = "style='display: none;'"; ?>
            <tr id="shipping_to_row">
              <th colspan="5" class="alignRight">
                <?php _e('Shipping to', 'simpleecommcart'); ?> <?php echo EcommerceCartSession::get('simpleecommcart_shipping_zip'); ?> 
                <?php
                    if (EcommerceCartSetting::getValue('international_sales')) {
                        echo EcommerceCartSession::get('simpleecommcart_shipping_country_code');
                    }
?>
                (<a href="#" id="change_shipping_zip_link">change</a>)
                &nbsp;
                <?php
                    $liveRates = EcommerceCartSession::get('EcommerceCartCart')->getLiveRates();
                    $rates = $liveRates->getRates();
                    EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] LIVE RATES: " . print_r($rates, true));
                    $selectedRate = $liveRates->getSelected();
                    $shipping = EcommerceCartSession::get('EcommerceCartCart')->getShippingCost();
?>
                <select name="live_rates" id="live_rates">
                  <?php foreach ($rates as $rate): ?>
                    <option value='<?php echo $rate->service ?>' <?php if ($selectedRate->service == $rate->service) {
...
?>
          ...
          <tr id="set_shipping_zip_row" <?php echo $zipStyle; ?>>
            <th colspan="5" class="alignRight"><?php _e('Enter Your Zip Code', 'simpleecommcart'); ?>:
              ...
              <?php if (EcommerceCartSetting::getValue('international_sales')): ?>
                <select name="shipping_country_code">
                  <?php
                    $customCountries = EcommerceCartCommon::getCustomCountries();
...
?>
              ...
              <?php
                ...
                <input type="hidden" name="shipping_country_code" value="<?php echo EcommerceCartCommon::getHomeCountryCode(); ?>" id="shipping_country_code">
              <?php
                endif; ?>
        ...
        <?php
            else: // Cart in read mode
                 ?>
          <tr>
            <th colspan="5" class='alignRight'>
              <?php
                $liveRates = EcommerceCartSession::get('EcommerceCartCart')->getLiveRates();
                if ($liveRates && EcommerceCartSession::get('simpleecommcart_shipping_zip') && EcommerceCartSession::get('simpleecommcart_shipping_country_code')) {
                    $selectedRate = $liveRates->getSelected();
                    echo __("Shipping to", "simpleecommcart") . " " . EcommerceCartSession::get('simpleecommcart_shipping_zip') . " " . __("via", "simpleecommcart") . " " . $selectedRate->service;
                }
?>
            </th>
          </tr>
        <?php
            endif; // End cart in read mode
             ?>
          ...
          <th colspan='5' class="alignRight"><?php _e('Shipping Method', 'simpleecommcart'); ?>: &nbsp;
            <select name='shipping_method_id' id='shipping_method_id'>
              ...
              <option value='<?php echo $id ?>' 
               <?php echo ($id == EcommerceCartSession::get('EcommerceCartCart')->getShippingMethodId()) ? 'selected' : ''; ?>><?php echo $name ?></option>
              <?php
                endforeach; ?>
            </select>
          </th>
      ...
      <!--  <tr>
          <th colspan='5' class="alignRight"><?php _e('Shipping Method', 'simpleecommcart'); ?>: 
            <?php
                $method = new SimpleEcommCartShippingMethod(EcommerceCartSession::get('EcommerceCartCart')->getShippingMethodId());
                echo $method->name;
?>
          </th>
        </tr>-->
	...
	<?php if (EcommerceCartSession::get('EcommerceCartCart')->requireShippingAndBillingAddress()): ?>
		<?php if ($fullMode): ?>
	 		<tr id="shipping_and_billing_to_row" >
	        	<th colspan="5" class="alignRight">
					 
					<table style="border-bottom:none;">
					<?php if (EcommerceCartSession::get('EcommerceCartCart')->requireShipTo()): ?>
						<tr  style="border-bottom:none;">
							<td>
								Ship to: 
								<select title="country" id="shipping_country" name="shipping_country">
				                  <?php
                foreach (EcommerceCartCommon::getCountries(false) as $code => $name) {
                    if ($name == 'All Countries') continue;
                    $selected = '';
                    if (EcommerceCartSession::get('simpleecommcart_shipping_country') == "$code~$name") {
...
?>
								...
								<input  type="hidden" id="hdnShippingState" value="<?php echo EcommerceCartSession::get('simpleecommcart_shipping_state') ?>"/>
							 ...
							 <td align="right">
							 <?php if (EcommerceCartSession::get('EcommerceCartCart')->requireCalculateTaxOrShippingButton()): ?>
						 <?php
                    $button_text = EcommerceCartSession::get('EcommerceCartCart')->getCalculateTaxOrShippingButtonText();
?>
					...
					<?php if (EcommerceCartSession::get('EcommerceCartCart')->requireBillTo()): ?>
						<tr  style="border-bottom:none;">
							<td>
								Bill to:
                ...
                foreach (EcommerceCartCommon::getCountries(false) as $code => $name) {
                    if ($name == 'All Countries') continue;
                    $selected = '';
                    if (EcommerceCartSession::get('simpleecommcart_billing_country') == "$code~$name") {
                        $selected = 'selected="selected"';
                    }
                    echo "<option value=\"$code~$name\" $selected>$name</option>";
                }
?>
								...
								<input  type="hidden" id="hdnBillingState" value="<?php echo EcommerceCartSession::get('simpleecommcart_billing_state') ?>"/>
					...
					<?php if (EcommerceCartSession::get('EcommerceCartCart')->requireShipping() && EcommerceCartSession::get('EcommerceCartCart')->requireShippingVariation()): ?>
                ...
                foreach ($shipping_variations as $s_variation) {
                    ...
                    $val = $s_variation->id;
                    $selected = '';
                    if (EcommerceCartSession::get('simpleecommcart_shipping_variations') == "$val") {
                        $selected = 'selected="selected"';
                    }
...
?>
    ...
    <tr class="subtotal">
      ...
      <td class='strong' colspan="1"><?php echo ECOMMERCECART_CURRENCY_SYMBOL ?><?php echo number_format($subtotal, 2); ?></td>
    </tr>
    ...
    <tr class="shipping">
      ...
      <td colspan="1" class="strong"><?php echo ECOMMERCECART_CURRENCY_SYMBOL ?><?php echo $shipping ?></td>
    </tr>
    <?php
    endif; ?>
    ...
    <?php if ($tax > 0): ?>
      <tr class="tax">
        ...
        <td colspan="1" class="strong"><?php echo ECOMMERCECART_CURRENCY_SYMBOL ?><?php echo number_format($tax, 2); ?></td>
      </tr>
    <?php
    endif; ?>
    
      <tr class="total">
        <?php if (EcommerceCartSession::get('EcommerceCartCart')->getNonSubscriptionAmount() > 0): ?>
        <td class="alignRight" colspan='3'>
          <?php if ($fullMode && EcommerceCartCommon::activePromotions()): ?>
            <p class="haveCoupon"><?php _e('Do you have a coupon?', 'simpleecommcart'); ?>
            <div id="couponCode"> <?php if (EcommerceCartSession::get('EcommerceCartCart')->getPromoStatus() < 0): ?>
              <span class="promoMessage"><?php echo EcommerceCartSession::get('EcommerceCartCart')->getPromoMessage(); ?></span>
            <?php
            endif; ?><input type='text' name='couponCode' value='' /></div>
        ...
        <td colspan="1" class="alignRight strong EcommerceCartCartTotalLabel"><?php _e('Total', 'simpleecommcart'); ?>:</td>
        <td colspan="1" class="strong">
          <?php
    echo ECOMMERCECART_CURRENCY_SYMBOL;
    echo number_format(EcommerceCartSession::get('EcommerceCartCart')->getGrandTotal() + $tax, 2);
?>
        </td>
      </tr>
  </table>
  ...
  <div id="viewCartNav">
	  ...
	  <?php
        ...
        if ((EcommerceCartSetting::getValue('require_terms') != 1) || (EcommerceCartSetting::getValue('require_terms') == 1 && (isset($_POST['terms_acceptance']) || EcommerceCartSession::get("terms_acceptance") == "accepted"))):
            if (EcommerceCartSetting::getValue('require_terms') == 1) {
                EcommerceCartSession::set("terms_acceptance", "accepted", true);
            }
?>
        <?php
            $checkoutImg = false;
...
?>
	  <?php
            $terms_page_link = '#';
            if (EcommerceCartSetting::getValue('terms_and_condition') == 'yes' && EcommerceCartSetting::getValue('terms_and_condition_page') != NULL) {
                $terms_page_id = EcommerceCartSetting::getValue('terms_and_condition_page');
...
<div id="continueShopping" style="padding-left:5px;">
        <?php if ($cartImgPath): ?>
          <a href='<?php echo EcommerceCartSession::get('SimpleEcommCartLastPage'); ?>' class="EcommerceCartCartContinueShopping" ><img src='<?php echo $continueShoppingImg ?>' /></a>
        <?php
            else: ?>
          <a href='<?php echo EcommerceCartSession::get('SimpleEcommCartLastPage'); ?>' class="SimpleEcommCartButtonSecondary EcommerceCartCartContinueShopping" title="Continue Shopping"><?php _e('<< CONTINUE SHOPPING', 'simpleecommcart'); ?></a>
        <?php
            endif; ?>
	</div>
      <div id="checkoutShopping" style="padding-right:10px;"> 
	  	<select name="checkout_select" id="checkout_select"> 
			 <?php
            echo 'use_authorize_checkout:' . EcommerceCartSetting::getValue('use_authorize_checkout');
            if (EcommerceCartSetting::getValue('use_paypal_standard_checkout') == 'on') {
?>
						<option value="paypalCheckout">Paypal Checkout</option>
					<?php
            }
            /*if(EcommerceCartSetting::getValue('use_authorize_checkout') == 'on')
            {
            ?>
...
?>
    ...
    <div id="SimpleEcommCartCheckoutReplacementText">
        <?php echo EcommerceCartSetting::getValue('cart_terms_replacement_text'); ?>
    </div>
    <?php
        endif; ?>
	   ...
	   <?php
        if (EcommerceCartSetting::getValue('require_terms') == 1 && (!isset($_POST['terms_acceptance']) && EcommerceCartSession::get("terms_acceptance") != "accepted")) {
            echo EcommerceCartCommon::getView("pro/views/terms.php", array("location" => "EcommerceCartCartTOS"));
        }
?>
...
<?php
  ...
  <?php if ($cartImgPath): ?>
    <p><a href='<?php echo EcommerceCartSession::get('SimpleEcommCartLastPage'); ?>' title="Continue Shopping" class="EcommerceCartCartContinueShopping"><img alt="Continue Shopping" class="continueShoppingImg" src='<?php echo $continueShoppingImg ?>' /></a>
  <?php
    else: ?>
    <p><a href='<?php echo EcommerceCartSession::get('SimpleEcommCartLastPage'); ?>' class="SimpleEcommCartButtonSecondary" title="Continue Shopping"><?php _e('Continue Shopping', 'simpleecommcart'); ?></a>
  <?php
    endif; ?>
  </div>
  <?php
    EcommerceCartSession::get('EcommerceCartCart')->clearPromotion();
    EcommerceCartSession::drop("terms_acceptance");
?>
<?php
endif; ?>
...
/* <![CDATA[ */
  ...
  $jq('document').ready(function() {
    $jq('#shipping_method_id').change(function() {
      $jq('#EcommerceCartCartForm').submit();
    });
