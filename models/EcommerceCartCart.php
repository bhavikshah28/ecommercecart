<?php
class EcommerceCartCart {
    /**
     * An array of EcommerceCartCartItem objects
     */
    private $_items;
    private $_promotion;
    ...
    public function addToCart() {
        ...
        EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Adding item to cart: $itemId");
        $options = '';
        if (isset($_POST['options_1'])) {
            $options = EcommerceCartCommon::postVal('options_1');
        }
        if (isset($_POST['options_2'])) {
            $options.= '~' . EcommerceCartCommon::postVal('options_2');
        }
        ...
        if (isset($_POST['item_user_price'])) {
            ...
            EcommerceCartSession::set("userPrice_$itemId", $sanitizedPrice);
        }
        if (EcommerceCartProduct::confirmInventory($itemId, $options)) {
            $this->addItem($itemId, $itemQuantity, $options);
        } else {
            EcommerceCartCommon::log("Item not added due to inventory failure");
            wp_redirect($_SERVER['HTTP_REFERER']);
        }
        //set default shipping and billing country
        if (!EcommerceCartSession::get('simpleecommcart_shipping_country')) {
            if ($this->requireShipTo()) {
                $home_country = EcommerceCartSetting::getValue('home_country');
                $country = '';
                foreach (EcommerceCartCommon::getCountries(true) as $code => $name) {
                    $country = "$code~$name";
                    break;
                }
                if ($home_country != '00~All Countries') $country = $home_country;
                EcommerceCartSession::set('simpleecommcart_shipping_country', $country);
            }
        }
        if (!EcommerceCartSession::get('simpleecommcart_billing_country')) {
            if ($this->requireBillTo()) {
                ...
                if ($home_country != '00~All Countries') $country = $home_country;
                EcommerceCartSession::set('simpleecommcart_billing_country', $country);
            }
        }
        //reset promotion
    ...
    public function updateCart() {
        if (EcommerceCartCommon::postVal('updateCart') == 'Calculate Shipping') {
            EcommerceCartSession::set('simpleecommcart_shipping_zip', EcommerceCartCommon::postVal('shipping_zip'));
            EcommerceCartSession::set('simpleecommcart_shipping_country_code', EcommerceCartCommon::postVal('shipping_country_code'));
        }
        if (EcommerceCartCommon::postVal('updateCart') == 'Calculate Tax & Shipping' || EcommerceCartCommon::postVal('updateCart') == 'Calculate Tax' || EcommerceCartCommon::postVal('updateCart') == 'Calculate Shipping') {
            EcommerceCartSession::set('simpleecommcart_shipping_country', EcommerceCartCommon::postVal('shipping_country'));
            if (EcommerceCartCommon::postVal('shipping_country') == 'US~United States') {
                EcommerceCartSession::set('simpleecommcart_shipping_state', EcommerceCartCommon::postVal('shipping_state_usa'));
            } else {
                EcommerceCartSession::set('simpleecommcart_shipping_state', EcommerceCartCommon::postVal('shipping_state_canada'));
            }
            EcommerceCartSession::set('simpleecommcart_billing_country', EcommerceCartCommon::postVal('billing_country'));
            if (EcommerceCartCommon::postVal('billing_country') == 'US~United States') {
                EcommerceCartSession::set('simpleecommcart_billing_state', EcommerceCartCommon::postVal('billing_state_usa'));
            } else {
                EcommerceCartSession::set('simpleecommcart_billing_state', EcommerceCartCommon::postVal('billing_state_canada'));
            }
            EcommerceCartSession::set('simpleecommcart_shipping_variations', EcommerceCartCommon::postVal('shipping_variations'));
        }
        if (EcommerceCartCommon::postVal('updateCart') == 'UPDATE CART') {
            //reset promotion
            $this->_promotion = null;
        }
        if (EcommerceCartCommon::postVal('updateCart') == 'CLEAR CART') {
            //clear cart
    ...
    public function addItem($id, $qty = 1, $optionInfo = '', $formEntryId = 0) {
        ...
        $product = new EcommerceCartProduct($id);
        if ($product->id > 0) {
            $newItem = new EcommerceCartCartItem($product->id, $qty, $optionInfo->options, $optionInfo->priceDiff);
            if (($product->isSubscription() || $product->isMembershipProduct()) && ($this->hasSubscriptionProducts() || $this->hasMembershipProducts())) {
                ...
                EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] about to remove membership item");
                $this->removeMembershipProductItem();
            }
            if ($product->isGravityProduct()) {
                EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Product being added is a Gravity Product: $formEntryId");
                if ($formEntryId > 0) {
                    $newItem->addFormEntryId($formEntryId);
                    $this->_items[] = $newItem;
                }
            } else {
                EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Product being added is NOT a Gravity Product");
                $isNew = true;
    ...
    public function removeItem($itemIndex) {
        if (isset($this->_items[$itemIndex])) {
            ...
            if (count($this->_items) <= 1) {
                $this->_items = array();
                EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Reset the cart items array");
            } else {
                unset($this->_items[$itemIndex]);
                EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Did not reset the cart items array because the cart contains more than just a membership item");
            }
            EcommerceCartSession::touch();
    ...
    public function removeItemByProductId($productId) {
        foreach ($this->_items as $index => $item) {
            if ($item->getProductId() == $productId) {
                EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Removing item at index: $index");
    ...
    public function removeMembershipProductItem() {
        foreach ($this->_items as $item) {
            if ($item->isMembershipProduct() || $item->isSubscription()) {
                ...
                EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Removing membership item with product id: $productId");
    ...
    public function getSubscriptionAmount() {
        ...
        if ($subId = $this->getSpreedlySubscriptionId()) {
            ...
            if (!$subscription->hasFreeTrial()) {
                ...
                EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Subscription amount for subscription id: $subId = " . $subscription->amount);
            }
        }
        return $amount;
    }
    ...
    public function getTax($state = 'All Sales', $zip = null) {
        $tax = 0;
        $tax_settings = unserialize(EcommerceCartSetting::getValue('tax_settings'));
        //flat rate
    ...
    public function isInternationalShipping() {
        $shipping_country = EcommerceCartSession::get('simpleecommcart_shipping_country');
        $home_country = EcommerceCartSetting::getValue('home_country');
    ...
    public function getFlatRate() {
        ...
        if (EcommerceCartSetting::getValue('shipping_options_flat_rate_option') == '1') {
            ...
            if (!$hasMultipleTypeProduct) {
                ...
                foreach ($this->_items as $item) {
                    ...
                    if ($this->isInternationalShipping()) {
                        //apply international
                        if ($quantity > 1) {
                            $flat_rate+= $product->single_sihipping_cost_international + ($product->multiple_sihipping_cost_international) * ($quantity - 1);
                        } else {
                        ...
                        //apply local
                        if ($quantity > 1) {
                            $flat_rate+= $product->single_sihipping_cost + ($product->multiple_sihipping_cost) * ($quantity - 1);
                        } else {
        ...
        } else if (EcommerceCartSetting::getValue('shipping_options_flat_rate_option') == '2') {
            ...
            if ($physical_product_found) {
                if ($this->isInternationalShipping()) {
                    $flat_rate+= EcommerceCartSetting::getValue('shipping_options_flat_rate_option2_international');
                } else {
                    $flat_rate+= EcommerceCartSetting::getValue('shipping_options_flat_rate_option2_local');
                }
            }
        } else if (EcommerceCartSetting::getValue('shipping_options_flat_rate_option') == '3') {
            ...
            if ($this->isInternationalShipping()) {
                foreach ($this->_items as $item) {
                    ...
                    $flat_rate+= EcommerceCartSetting::getValue('shipping_options_flat_rate_option3_international') * $item->getQuantity();
                }
            } else {
                foreach ($this->_items as $item) {
                    $product = $item->getProduct();
                    if ($product->isDigital()) continue;
                    if (!$product->isShipped()) continue;
                    $flat_rate+= EcommerceCartSetting::getValue('shipping_options_flat_rate_option3_local') * $item->getQuantity();
                }
            }
        }
        return $flat_rate;
        ...
        if ($promotion->loadByCode($code)) {
            ...
            */
            if ($promotion->active == '1') {
                ...
                if ($promotion->redemption_limit > 0 && $promotion->redemption_limit > $promotion->redemption_count) {
    ...
    public function getDiscountAmount() {
        ...
        if (is_a($this->_promotion, 'SimpleEcommCartPromotion')) {
            ...
            // EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Getting discount Total: $total -- Discounted Total: $discountedTotal -- Discount: $discount");
    ...
    public function getDiscountAmountForSingleProduct($product_id) {
        ...
        if (is_a($this->_promotion, 'SimpleEcommCartPromotion')) {
            ...
            if ($discountedTotal < 0) {
                ...
                if ($total > $discountedTotal) $discount = number_format($total - $discountedTotal, 2, '.', '');
                else $discount = $discountedTotal;
            }
            // EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Getting discount Total: $total -- Discounted Total: $discountedTotal -- Discount: $discount");
    ...
    public function storeOrder($orderInfo) {
        ...
        EcommerceCartCommon::log('TAX____:' . $orderInfo['tax']);
        //////////////////////////////////////
        $order = new SimpleEcommCartOrder();
        $orderInfo['trans_id'] = (empty($orderInfo['trans_id'])) ? 'MT-' . EcommerceCartCommon::getRandString() : $orderInfo['trans_id'];
     ...
     * Note that this function returns the actual EcommerceCartProduct not the EcommerceCartCartItem.
     * If there is no membership product in the cart, return false.
     *
     * @return EcommerceCartProduct
     */
    public function getMembershipProduct() {
        $product = false;
        if ($item = $this->getMembershipProductItem()) {
            $product = new EcommerceCartProduct($item->getProductId());
        }
        return $product;
    }
    ...
    /**
     * Return the EcommerceCartCartItem with the PayPal subscription
     *
     * @return EcommerceCartCartItem
     */
    ...
    public function requireShippingVariation() {
        ...
        if (EcommerceCartSetting::getValue('shipping_variation_show') == '1') {
    ...
    public function setShippingMethod($id) {
        ...
        if ($method->load($id)) {
            ...
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Set shipping method id to: $id");
        }
    }
    ...
    public function getShippingMethodName() {
        // Look for live rates
        if (EcommerceCartSession::get('SimpleEcommCartLiveRates')) {
            $rate = EcommerceCartSession::get('SimpleEcommCartLiveRates')->getSelected();
            return $rate->service;
        }
    ...
    public function checkCartInventory() {
        ...
        foreach ($this->_items as $itemIndex => $item) {
            if (!EcommerceCartProduct::confirmInventory($item->getProductId(), $item->getOptionInfo(), $item->getQuantity())) {
                EcommerceCartCommon::log("Unable to confirm inventory when checking cart.");
                $qtyAvailable = EcommerceCartProduct::checkInventoryLevelForProduct($item->getProductId(), $item->getOptionInfo());
                if ($qtyAvailable > 0) {
    ...
    public function getLiveRates() {
        EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Call to getLiveRates");
        if (!SIMPLEECOMMCART_PRO) {
            return false;
        }
        $weight = EcommerceCartSession::get('SimpleEcommCartCart')->getCartWeight();
        $zip = EcommerceCartSession::get('simpleecommcart_shipping_zip') ? EcommerceCartSession::get('simpleecommcart_shipping_zip') : false;
        $countryCode = EcommerceCartSession::get('simpleecommcart_shipping_country_code') ? EcommerceCartSession::get('simpleecommcart_shipping_country_code') : EcommerceCartCommon::getHomeCountryCode();
        // Make sure _liveRates is a SimpleEcommCartLiveRates object
        if (get_class($this->_liveRates) != 'SimpleEcommCartLiveRates') {
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] WARNING: \$this->_liveRates is not a SimpleEcommCartLiveRates object so we're making it one now.");
        ...
        if (EcommerceCartSession::get('SimpleEcommCartLiveRates') && get_class($this->_liveRates) == 'SimpleEcommCartLiveRates') {
            $cartWeight = $this->getCartWeight();
            $this->_liveRates = EcommerceCartSession::get('SimpleEcommCartLiveRates');
            $liveWeight = $this->_liveRates->weight;
            $liveZip = $this->_liveRates->toZip;
            $liveCountry = $this->_liveRates->getToCountryCode();
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] 
        $liveWeight == $weight
        $liveZip == $zip
      ...
      ");
            if ($this->_liveRates->weight == $weight && $this->_liveRates->toZip == $zip && $this->_liveRates->getToCountryCode() == $countryCode) {
                EcommerceCartCommon::log("Using Live Rates from the session: " . $this->_liveRates->getSelected()->getService());
                return EcommerceCartSession::get('SimpleEcommCartLiveRates');
            }
        }
        if ($this->getCartWeight() > 0 && EcommerceCartSession::get('simpleecommcart_shipping_zip') && EcommerceCartSession::get('simpleecommcart_shipping_country_code')) {
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Clearing current live shipping rates and recalculating new rates.");
            $this->_liveRates->clearRates();
            $this->_liveRates->weight = $weight;
            $this->_liveRates->toZip = $zip;
            $method = new SimpleEcommCartShippingMethod();
            // Get USPS shipping rates
            if (EcommerceCartSetting::getValue('usps_username')) {
            ...
            if (EcommerceCartSetting::getValue('ups_apikey')) {
        ...
        EcommerceCartSession::set('SimpleEcommCartLiveRates', $this->_liveRates);
        EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Dump live rates: " . print_r($this->_liveRates, true));
    ...
    public function getUspsRates() {
        ...
        $fromZip = EcommerceCartSetting::getValue('usps_ship_from_zip');
        $toZip = EcommerceCartSession::get('simpleecommcart_shipping_zip');
        EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Getting rates for USPS: $fromZip > $toZip > $weight");
    ...
    public function getUspsIntlRates($countryCode) {
        ...
        $zipOrigin = EcommerceCartSetting::getValue('usps_ship_from_zip');
        EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Getting rates for USPS Intl: $zipOrigin > $countryCode > $value > $weight");
    ...
    public function getUpsRates() {
        ...
        $weight = EcommerceCartSession::get('SimpleEcommCartCart')->getCartWeight();
        $zip = EcommerceCartSession::get('simpleecommcart_shipping_zip');
        $countryCode = EcommerceCartSession::get('simpleecommcart_shipping_country_code');
    ...
    protected function _setShippingMethodFromPost() {
        ...
        if (isset($_POST['shipping_method_id'])) {
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Not using live shipping rates");
        ...
        elseif (isset($_POST['live_rates'])) {
            if (EcommerceCartSession::get('SimpleEcommCartLiveRates')) {
                EcommerceCartSession::get('SimpleEcommCartLiveRates')->setSelected($_POST['live_rates']);
                // EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] This LIVE RATE is now set: " . EcommerceCartSession::get('SimpleEcommCartLiveRates')->getSelected()->getService());
                // EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Using live shipping rates to set shipping method from post: " . $_POST['live_rates']);
                
            }
        }
    }
    protected function _updateQuantitiesFromPost() {
        $qtys = EcommerceCartCommon::postVal('quantity');
        if (is_array($qtys)) {
            foreach ($qtys as $itemIndex => $qty) {
                $item = $this->getItem($itemIndex);
                if (!is_null($item) && get_class($item) == 'EcommerceCartCartItem') {
                    if ($qty == 0) {
                        EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Customer specified quantity of 0 - remove item.");
                        $this->removeItem($itemIndex);
                    }
                    if (EcommerceCartProduct::confirmInventory($item->getProductId(), $item->getOptionInfo(), $qty)) {
                        ...
                        if (!EcommerceCartSession::get('SimpleEcommCartInventoryWarning')) {
                            EcommerceCartSession::set('SimpleEcommCartInventoryWarning', '');
                        }
                        $inventoryWarning = EcommerceCartSession::get('SimpleEcommCartInventoryWarning');
                        ...
                        EcommerceCartSession::set('SimpleEcommCartInventoryWarning', $inventoryWarning);
                        EcommerceCartCommon::log("Quantity available ($qtyAvailable) cannot meet desired quantity ($qty) for product id: " . $item->getProductId());
                    }
                }
            }
        }
    ...
    protected function _setCustomFieldInfoFromPost() {
        ...
        $custom = EcommerceCartCommon::postVal('customFieldInfo');
        if (is_array($custom)) {
    ...
    protected function _setPromoFromPost() {
        if (isset($_POST['couponCode']) && $_POST['couponCode'] != '') {
            $couponCode = EcommerceCartCommon::postVal('couponCode');
    ...
    foreach($options as $opt) {
      EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Working with option: $opt");
      if(preg_match('/\+\s*\$/', $opt)) {
    ...
    */
    protected function _processOptionInfo($optionInfo) {
        ...
        foreach ($options as $opt) {
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Working with option: $opt");
            $pprice = '/\+\s*\\' . ECOMMERCECART_CURRENCY_SYMBOL . '/';
            $pcurrency = '+' . ECOMMERCECART_CURRENCY_SYMBOL;
            $mprice = '/-\s*\\' . ECOMMERCECART_CURRENCY_SYMBOL . '/';
            $mcurrency = '-' . ECOMMERCECART_CURRENCY_SYMBOL;
