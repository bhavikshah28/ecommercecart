...
class EcommerceCartSession {
    ...
    public static function touch() {
        if (self::$_data['id'] > 0) {
            // EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Touching the session: " . self::$_data['session_id']);
            self::_save();
        }
    }
    ...
    public function dump() {
        $out = "Ecommerce Cart Session Dump:\n\n";
    ...
    protected static function _init() {
        if (!self::$_started) {
            self::$_started = true;
            self::$_data = array('id' => null, 'session_id' => 0, 'ip_address' => '', 'user_agent' => '', 'last_activity' => '', 'user_data' => '');
            self::$_userData = array();
    ...
    protected function _start() {
        ...
        if (strpos(basename($url), '.')) {
            ...
            if (in_array($ext, $ignoreList)) {
                ...
                EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Not starting session for this request: $url");
            }
        }
        if (self::$_validRequest) {
            $sid = isset($_COOKIE['EcommerceCartSID']) ? $_COOKIE['EcommerceCartSID'] : false;
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] ***************************************************\n" . "Starting session with EcommerceCartSID: $sid\nREQUEST: " . $_SERVER['REQUEST_URI'] . "\nQUERY STRING: " . $_SERVER['QUERY_STRING']);
    ...
    protected static function _loadSession($sessionId = false) {
        global $wpdb;
        $tableName = EcommerceCartCommon::getTableName('sessions');
        $loaded = false;
        if ($sessionId) {
        ...
        if (!$loaded) {
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] _loadSession() was unable to load the session id: $sessionId");
            self::_newSession();
        } else {
            // EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Successfully loaded session: $sessionId");
    ...
    protected static function _newSession() {
        EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Creating a new session");
        $userAgent = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : 'noUserAgent';
        $data = array('id' => null, 'session_id' => self::_newSessionId(), 'ip_address' => $_SERVER['REMOTE_ADDR'], 'user_agent' => $userAgent, 'last_activity' => date('Y-m-d H:i:s', EcommerceCartCommon::localTs()), 'user_data' => serialize(self::$_userData));
        self::$_data = $data;
        self::_save();
        // setcookie('SimpleEcommCartSID', self::$_data['session_id'], 0, '/', self::_getDomain());
        setcookie('EcommerceCartSID', self::$_data['session_id'], 0, '/');
    ...
    public static function _isActive($data = null) {
        ...
        $isActive = false;
        if ($data['id'] > 0) {
            $expireTs = strtotime('-' . self::$_maxLifetime . ' minutes', EcommerceCartCommon::localTs());
            $lastActiveTs = strtotime($data['last_activity'], EcommerceCartCommon::localTs());
            // EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] $lastActiveTs > $expireTs Last activity: " . $data['last_activity']);
    ...
    protected static function _isValid($data) {
        ...
        if ($data['ip_address'] != $_SERVER['REMOTE_ADDR']) {
            $isValid = false;
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Session is not valid - IP Address changed");
        } elseif ($data['user_agent'] != $_SERVER['HTTP_USER_AGENT']) {
            $isValid = false;
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Session is not valid - User agent changed");
        } elseif (!self::_isActive($data)) {
            $isValid = false;
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Session is not valid - Expired");
        }
        return $isValid;
    }
    /**
    ...
    protected function _newSessionId() {
        global $wpdb;
        do {
            $sessionId = EcommerceCartCommon::getRandString(40);
            $tableName = EcommerceCartCommon::getTableName('sessions');
    ...
    protected function _deleteExpiredSessions() {
        global $wpdb;
        $tableName = EcommerceCartCommon::getTableName('sessions');
        $cutOffDate = date('Y-m-d H:i:s', strtotime('-' . self::$_maxLifetime . ' minutes', EcommerceCartCommon::localTs()));
    ...
    protected static function _save() {
        if (self::$_validRequest) {
            ...
            self::$_data['last_activity'] = date('Y-m-d H:i:s', EcommerceCartCommon::localTs());
            //EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Saving Session User Data: " . print_r(self::$_userData, true));
            self::$_data['id'] > 0 ? self::_update() : self::_insert();
        } else {
            $reqInfo = "\nEcommerceCartSID: $sid\nREQUEST: " . $_SERVER['REQUEST_URI'] . "\nQUERY STRING: " . $_SERVER['QUERY_STRING'];
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Not saving session because the request is being ignored $reqInfo");
        }
    }
    protected static function _insert() {
        global $wpdb;
        $wpdb->insert(EcommerceCartCommon::getTableName('sessions'), self::$_data);
        self::$_data['id'] = $wpdb->insert_id;
    }
    protected static function _update() {
        global $wpdb;
        $wpdb->update(EcommerceCartCommon::getTableName('sessions'), self::$_data, array('id' => self::$_data['id']));
    }
