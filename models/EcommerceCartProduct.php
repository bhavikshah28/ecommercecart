<?php
class EcommerceCartProduct extends EcommerceCartModelAbstract {
    protected $_creditAmount;
    public function __construct($id = null) {
        $this->_tableName = EcommerceCartCommon::getTableName('products');
    ...
    public function loadByDuid($duid) {
        $itemsTable = EcommerceCartCommon::getTableName('order_items');
    ...
    public function countDownloadsForDuid($duid) {
        $downloadsTable = EcommerceCartCommon::getTableName('downloads');
    ...
    public static function checkInventoryLevelForProduct($id, $variation = '') {
        ...
        $p = new EcommerceCartProduct($id);
        $ikey = $p->getInventoryKey($variation);
        $count = $p->getInventoryCount($ikey);
        //EcommerceCartCommon::log("Check Inventory Level For Product: $ikey = $count");
        return $count;
    }
    public static function decrementInventory($id, $variation = '', $qty = 1) {
        EcommerceCartCommon::log("Decrementing Inventory: line " . __LINE__);
        // Build varation ikey string component
        if (!empty($variation)) {
            $variation = self::scrubVaritationsForIkey($variation);
        }
    ...
    public static function confirmInventory($id, $variation = '', $desiredQty = 1) {
        EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Confirming Inventory:\n$id | $variation | $desiredQty");
        $ok = true;
        $setting = new EcommerceCartSetting();
        $trackInventory = EcommerceCartSetting::getValue('track_inventory');
        if ($trackInventory == 1) {
            $p = new EcommerceCartProduct($id);
            $variation = self::scrubVaritationsForIkey($variation);
            $ikey = $p->getInventoryKey($variation);
            if ($p->isInventoryTracked($ikey)) {
                ...
                $out_of_stock_notification = EcommerceCartSetting::getValue("out_of_stock_notification");
                if ($out_of_stock_notification == '1') {
                    $out_of_stock_threshhold = EcommerceCartSetting::getValue("out_of_stock_threshhold") + 0;
                ...
                EcommerceCartCommon::log("Inventory not tracked: $ikey");
            }
        }
        return $ok;
    }
    ...
    public function insertInventoryData() {
        ...
        foreach ($keys as $key) {
            $inventory = EcommerceCartCommon::getTableName('inventory');
    ...
    public function updateInventoryFromPost($ikey) {
        $inventory = EcommerceCartCommon::getTableName('inventory');
        $track = EcommerceCartCommon::postVal("track_$ikey");
        $qty = EcommerceCartCommon::postVal("qty_$ikey");
        $sql = "UPDATE $inventory set track=%d, quantity=%d where ikey=%s";
        $sql = $this->_db->prepare($sql, $track, $qty, $ikey);
        $this->_db->query($sql);
        EcommerceCartCommon::checkAndSendStockNotification($ikey);
    }
    ...
    public function isInventoryTracked($ikey) {
        ...
        //EcommerceCartCommon::log("Is inventory tracked query: $sql");
    ...
    public function pruneInventory(array $ikeyList) {
        ...
        $this->_db->query($sql);
        //EcommerceCartCommon::log("Prune Inventory: $sql");
        ...
        if ($optNumber == 1) {
            ...
            if (count($variation1s) > 0) {
                ...
                foreach ($variation1s as $v) {
                    ...
                    if ($price > 0) $opt = trim($v . ' ' . $signs1s[$i] . ' ' . ECOMMERCECART_CURRENCY_SYMBOL . $price);
                    else $opt = trim($v);
            ...
            if (count($variation2s) > 0) {
                ...
                foreach ($variation2s as $v) {
                    ...
                    if ($price > 0) $opt = trim($v . ' ' . $signs2s[$i] . ' ' . ECOMMERCECART_CURRENCY_SYMBOL . $price);
                    else $opt = trim($v);
                    $select.= "\n\t<option value=\"" . htmlentities($opt) . "\">$opt</option>";
                    $i++;
                }
    ...
    public function getShippingPrice($methodId) {
        ...
        $ratesTable = EcommerceCartCommon::getTableName('shipping_rates');
        ...
        if ($rate === NULL) {
            ...
            $shippingMethods = EcommerceCartCommon::getTableName('shipping_methods');
    ...
    public static function getProductIdByGravityFormId($id) {
        global $wpdb;
        $products = EcommerceCartCommon::getTableName('products');
    ...
    public function subscriptionMismatch() {
        ...
        if ($this->isSpreedlySubscription() && $this->isPayPalSubscription()) {
            if (class_exists(SpreedlySubscription) && class_exists(SimpleEcommCartPayPalSubscription)) {
                ...
                EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Comparing: $pp <--> $sp");
                if ($pp != $sp) {
                    $ok = true;
                }
            }
    ...
    public function getSalesForMonth($month, $year) {
        $orders = EcommerceCartCommon::getTableName('orders');
        $orderItems = EcommerceCartCommon::getTableName('order_items');
    ...
    public function validate() {
        ...
        if (!empty($this->download_path)) {
            $dir = EcommerceCartSetting::getValue('product_folder');
    ...
    public function gravityGetVariationPrices($gfEntry) {
        $options = array();
        EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Gravity Forms Entry:  " . print_r($gfEntry, true));
        ...
        if (strlen($_FILES['product']['tmp_name']['upload']) > 2) {
            ...
            if ($dir) {
                ...
                if (move_uploaded_file($src, $path)) {
                    ...
                    throw new EcommerceCartException($msg, 66101);
                }
            }
        }
    ...
    public function getPriceDescription($priceDifference = 0) {
        if ($this->id > 0) {
            ...
            if ($price > 0) {
                $priceDescription = ECOMMERCECART_CURRENCY_SYMBOL . number_format($price, 2);
            }
            // if($this->hasFreeTrial()) {
            ...
            // }
            //  else {
            // EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Product custom price description: $this->priceDes");
            if (!empty($this->priceDescription)) {
                $priceDescription = $this->priceDescription;
            } else {
                //$priceDescription = ECOMMERCECART_CURRENCY_SYMBOL . number_format($this->price + $priceDifference, 2);
                $priceDescription = $this->priceDescription;
            }
            //}
    ...
    public function getProRateInfo() {
        $data = false;
        if ($this->isSpreedlySubscription()) {
            if (EcommerceCartCommon::isLoggedIn() && EcommerceCartSession::get('EcommCartCart')) {
                if ($subscriptionId = EcommerceCartSession::get('EcommerceCartCart')->getSpreedlySubscriptionId()) {
                    try {
                        $invoiceData = array('subscription-plan-id' => $subscriptionId, 'subscriber' => array('customer-id' => EcommerceCartSession::get('SimpleEcommCartAccountId')));
                        ...
                        $data->money = ECOMMERCECART_CURRENCY_SYMBOL . number_format($this->_creditAmount, 2);
                        EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] PRICE DESCRIPTON CREDIT: $proRated");
                        EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Spreedly Invoice: " . print_r($invoice->invoiceData, true));
                    }
                    catch(SpreedlyException $e) {
                        EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Unable to locate spreedly customer: " . EcommerceCartSession::get('SimpleEcommCartAccountId'));
                    }
                }
            }
        }
     ...
     * Throw EcommerceCartException if the save fails.
     *
     * @return int The product id
     * @throws EcommerceCartException on save failure
     */
    public function save() {
        ...
        if (count($errors)) {
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] " . get_class($this) . " save errors: " . print_r($errors, true));
            $this->setErrors($errors);
            $errors = print_r($errors, true);
            throw new EcommerceCartException('Product save failed: ' . $errors, 66102);
        }
        return $productId;
    }
    ...
    public function saveInventoryData($quantities) {
        ...
        $sql = "DELETE from " . EcommerceCartCommon::getTableName('inventory') . " ";
        $this->_db->query($sql);
        foreach ($quantities as $key => $value) {
            $items = array_filter(explode("_", $key));
            $product_id = $items[0];
            $sql = "INSERT INTO " . EcommerceCartCommon::getTableName('inventory') . " (ikey,product_id,quantity,track) values('$key',$product_id,$value,'1')";
            $this->_db->query($sql);
        }
    }
        ...
        if ($data["selection"] == "all") {
            ...
            foreach ($checks as $key => $value) {
                if ($value == "on") {
                    $items = array_filter(explode("_", $key));
                    $product_id = $items[1];
                    $p = new EcommerceCartProduct($product_id);
                    $basePrice = $p->price;
                    if ($data["type"] == "$") {
                        if ($data["increment"] == "increase") {
                            $basePrice+= $data["amount"];
    ...
    public function getInventoryData() {
        $sql = "SELECT ikey,quantity from " . EcommerceCartCommon::getTableName('inventory') . " ";
    ...
    public function getInventoryDataForSpecificProduct($id) {
        $sql = "SELECT ikey,quantity from " . EcommerceCartCommon::getTableName('inventory') . " where product_id=" . $id . "";
        $sql = $this->_db->prepare($sql, null);
        $data = $this->_db->get_results($sql, OBJECT_K);
        return $data;
    }
    ...
    public function getMaximumProductId() {
        $sql = "SELECT MAX( id ) FROM " . EcommerceCartCommon::getTableName('products');
        $query = $this->_db->prepare($sql, $this->id);
        $num = $this->_db->get_var($query);
        return $num;
    }
