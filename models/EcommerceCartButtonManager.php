<?php
class EcommerceCartButtonManager {

  /**
   * Return the HTML for rendering the add to cart buton for the given product id
   */
  public static function getCartButton(EcommerceCartProduct $product, $attrs) {
    ...
    if($product->id > 0) {
      ...
      if($showPrice == 'yes' || $showPrice == 'only') {
        $price = ECOMMERCECART_CURRENCY_SYMBOL . number_format($product->price, 2);
        
        // Check for subscription pricing
    //     if($product->isSubscription()) {
    //       if($product->isPayPalSubscription()) {
    //         EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Rendering button for PayPal subscription");
    ...
    //     }
      ...
      $view = EcommerceCartCommon::getView('views/cart-button.php', $data);
    }
    return $view;
  }
  ...
  public function getAddToCartImagePath($attrs) {
    ...
    if(isset($attrs['img'])) {
      ...
      //if($cartImgPath) {
        $cartImgPath = EcommerceCartCommon::endSlashPath($cartImgPath);
