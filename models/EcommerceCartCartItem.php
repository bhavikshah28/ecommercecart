...
class EcommerceCartCartItem {
    ...
    public function __construct($productId = 0, $qty = 1, $optionInfo = '', $priceDifference = 0) {
        ...
        // EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] New Cart Item Option Info: $optionInfo");
    ...
    public function getProduct() {
        $product = new EcommerceCartProduct($this->_productId);
        return $product;
    }
    ...
    public function getOptionInfo() {
        ...
        if ($this->isSubscription()) {
            ...
            //EcommerceCartCommon::log("This is a subscription product: $options " . $this->getItemNumber());
                ...
                if ($product->gravity_form_id > 0) {
                    ...
                    if (count($this->_formEntryIds) == 0) {
                        ...
                        if ($product->gravity_form_qty_id > 0) {
                            ...
                            $gr = new SimpleEcommCartGravityReader();
    ...
    public function getCustomField($itemIndex, $fullMode = true) {
        ...
        if ($this->_productId > 0) {
          ...
          <br/><p class=\"SimpleEcommCartCustomFieldDesc\">$desc:<br/><strong>$value</strong> $change</p>
          <div id='customForm_$itemIndex' style='display: $showCustomForm;'>
          <input type=\"text\" name=\"customFieldInfo[$itemIndex]\" value=\"$value\" class=\"SimpleEcommCartCustomTextField\" id=\"custom_field_info_$itemIndex\" />
          <input type=\"submit\" value=\"$buttonValue\" /></div>";
                    ...
                    $out = "<br/><p class=\"SimpleEcommCartCustomFieldDesc\">$desc:<br/><strong>$value</strong></p>";
                }
          ...
          <br/><p class=\"SimpleEcommCartCustomFieldDesc\">$desc:<br/><strong>$brValue</strong><br/>$change</p>
          <div id='customForm_$itemIndex' style='display: $showCustomForm;'>
          <textarea name=\"customFieldInfo[$itemIndex]\" class=\"SimpleEcommCartCustomTextarea\" id=\"custom_field_info_$itemIndex\" />$value</textarea>
    ...
    public function getProductPrice() {
        if ($this->_productId > 0) {
            ...
            if ($this->isPayPalSubscription()) {
                ...
                if (EcommerceCartSession::get("userPrice_$this->_productId")) {
                    // using a user-defined price
                    $userPrice = EcommerceCartSession::get("userPrice_$this->_productId");
        ...
        if ($this->_productId > 0) {
            ...
            if ($product->isPayPalSubscription()) {
                ...
                if (EcommerceCartSession::get("userPrice_$this->_productId")) {
                    ...
                    $priceDescription = SIMPLEECOMMCART_CURRENCY_SYMBOL . number_format($userPrice, 2);
                } else {
    ...
    public function isEqual(EcommerceCartCartItem $item) {
        $isEqual = true;
    ...
    public function showAttachedForms($fullMode) {
        ...
        if (is_array($this->_formEntryIds)) {
            foreach ($this->_formEntryIds as $entryId) {
                ...
                $removeLink .= 'simpleecommcart-task=remove-attached-form&entry=' . $entryId;
                $removeLink = '<a class="SimpleEcommCartRemoveFormLink" href="' . $removeLink . '">remove</a>';
                }
                */
                $out.= "<div class='SimpleEcommCartGravityFormDisplay'>" . SimpleEcommCartGravityReader::displayGravityForm($entryId) . "</div>";
            }
        }
        return $out;
    }
    public function detachFormEntry($lead_id) {
        ...
        EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Start to detach gravity forms: " . print_r($entries, true));
        if (in_array($lead_id, $entries)) {
            EcommerceCartCommon::log('[' . basename(__FILE__) . ' - line ' . __LINE__ . "] Lead id is in the list of entries: " . $lead_id);
